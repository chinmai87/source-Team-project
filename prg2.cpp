...
public:
    ...
    void display() const {
        cout << left << setw(10) << memberId << setw(30) << name << endl;
    }
};
...
private:
    ...
    const string memberFile = "members.dat";
...
public:
    ...
    void searchBook() {
        ...
        for (const auto& b : books) {
            ...
            if (t.find(keyword) != string::npos || a.find(keyword) != string::npos) {
                b.display();
                found = true;
            }
        }
    ...
    void viewMembers() {
        cout << left << setw(10) << "Member ID" << setw(30) << "Name" << endl;
        for (const auto& m : members)
            m.display();
    }
};
